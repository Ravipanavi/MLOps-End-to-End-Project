...
class SimpleStorageService:
    ...
    @staticmethod
    def read_object(s3_object: object, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
        """
        Reads the specified S3 object with optional decoding and formatting.

        Args:
            s3_object (object): The S3 object to read from.
        ...
        """
        try:
            logging.info("Reading S3 object content.")
    ...
    def get_file_object(self, filename: str, bucket_name: str) -> object:
        ...
        try:
            ...
            if not file_objects:
                raise FileNotFoundError(f"No file found with prefix '{filename}' in bucket '{bucket_name}'")
            if len(file_objects) > 1:
                logging.warning(f"Multiple files found with prefix '{filename}'. Returning the first one.")
